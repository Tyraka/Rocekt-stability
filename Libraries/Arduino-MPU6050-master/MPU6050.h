...
class MPU6050
{
	...
	void calibrateAccel(uint8_t samples = 50);
	void setThreshold(uint8_t multiple = 1);
	void setThreshold_2(uint8_t multiple = 1);
	uint8_t getThreshold(void);
	uint8_t getThreshold_2(void);
	...
	Vector ta, da;
	Vector th,th_2;     // Threshold
	Activites a;   // Activities

	float dpsPerDigit, rangePerDigit;
	float actualThreshold;
	float actualThreshold_2;
	bool useCalibrate;
	bool kalibracja;
	int mpuAddress;
